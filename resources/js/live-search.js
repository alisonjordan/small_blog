  ...
  renderResultsHTML(posts) {
    if (posts.length) {
      ...
      ${posts
        ...
        <img class="avatar-tiny" src="${post.author_post.avatar}"> <strong>${post.title}</strong>
        <span class="text-muted small">by ${post.author_post.username} on ${postDate.getMonth() + 1}/${postDate.getDate()}/${postDate.getFullYear()}</span>
      </a>`
        })
        .join("")}
    </div>`)
